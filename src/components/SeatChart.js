...
const SeatChart = ({ occasion, ticketChain, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await ticketChain.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await ticketChain.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
